...
public class AutoAirDrop extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (!locFile.exists()) {
			...
			copy(getResource("locations.yml"), locFile);
		}
		try {
			locYml.load(this.locFile);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (!new File(File.separator + getDataFolder(), "config.yml").exists()) {
			saveDefaultConfig();
		}
		loadConfig();
	...
	public void runTask() {
		if (ALLOW && PluginOn) {
			new AutoAD().runTaskTimer(this, Delay, Delay);
		}
	}
	...
	public void loadConfig() {
		...
		String error = ChatColor.RED + "============ Error in " + ChatColor.AQUA + "Auto AirDrop " + ChatColor.RED
				+ "config.yml File ============";
		String st = ChatColor.YELLOW + " ";
		String fi = ChatColor.RED + "===============================================================";
		ALLOW = true;
		if (!getServer().getWorlds().contains(getServer().getWorld(World))) {
			ALLOW = false;
			PluginOn = false;
			console.sendMessage(error);
			console.sendMessage(st + "World not found.");
			console.sendMessage(st + "Auto AirDrop Disabled.");
			console.sendMessage(fi);
		}
		if (ALLOW && Delay < 1 || ALLOW && Delay != (int) Delay) {
			ALLOW = false;
			PluginOn = false;
			console.sendMessage(error);
			console.sendMessage(st + "Wrong Delay.");
			console.sendMessage(st + "Auto AirDrop Disabled.");
			console.sendMessage(fi);
		}
		if (ALLOW && ModArea == ModLoc && ModArea) {
			ALLOW = false;
			PluginOn = false;
			console.sendMessage(error);
			console.sendMessage(st + "Both AirDrop Mode are Enabled. Disable One");
			console.sendMessage(st + "Auto AirDrop Disabled.");
			console.sendMessage(fi);
		}
		if (ALLOW && ModArea == ModLoc && !ModArea) {
			...
			PluginOn = false;
			console.sendMessage(error);
			console.sendMessage(st + "Both AirDrop Mode are Disabled. Enable One");
			console.sendMessage(st + "Auto AirDrop Disabled.");
			console.sendMessage(fi);
		}
		if (ALLOW) {
	...
	public void run() {
		...
		if (ModArea) {
			...
			if (!c.contains(S + ".Maximum X") || !c.contains(S + ".Minimum X") || !c.contains(S + ".Maximum Z")
					|| !c.contains(S + ".Minimum Z")) {
				ALLOW = false;
				PluginOn = false;
				console.sendMessage(error);
			...
			if (MaxX < MinX || MaxX == MinX) {
				...
				console.sendMessage(error);
				console.sendMessage(
						ChatColor.AQUA + " [" + name + "]" + st + "Maximum X can't be lower or equal to Minimum X");
				console.sendMessage(st + "Auto AirDrop Disabled.");
				console.sendMessage(fi);
				return;
			}
			if (MaxZ < MinZ || MaxZ == MinZ) {
				...
				console.sendMessage(error);
				console.sendMessage(
						ChatColor.AQUA + " [" + name + "]" + st + "Maximum Z can't be lower or equal to Minimum Z");
				console.sendMessage(st + "Auto AirDrop Disabled.");
				console.sendMessage(fi);
				return;
			}
		...
		String addloc = Integer.toString(X) + "-" + Integer.toString(Y) + "-" + Integer.toString(Z);
		LocList.add(addloc);
		s.broadcastMessage(ChatColor.WHITE + "[" + ChatColor.DARK_RED + World + ChatColor.WHITE + "] "
				+ ChatColor.DARK_GREEN + "AirDrop dropped at: " + ChatColor.AQUA + "X: " + ChatColor.RED + X
				+ ChatColor.AQUA + " Z: " + ChatColor.RED + Z);
	}
